...
- **Temporary hotfix key:** until transport ownership is narrowed further, the dedupe key is `event_id + transport + canonical payload fingerprint`. This prevents a cloud copy from suppressing the corresponding local copy, drops exact same-transport redelivery, and still admits a same-id semantic enrichment such as write-once provider-usage reconciliation.
...
- **No same-transport re-broadcast loop:** the deduplicator drops only exact repeated deliveries on the same transport. A changed payload under the same canonical identity is processed once; local and cloud copies are both allowed through during this temporary mitigation.
