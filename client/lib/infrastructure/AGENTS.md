...
- **Desktop Authentication Exchange:** The local transport may carry only the exact credential-free `authentication_exchange` notification. It cannot carry or request auth state; receivers reload owner-only `auth.json`. Native auth mutations are serialized through the stable owner-only `auth.refresh.lock`; Mobile and Web never start this exchange or open the lock.
