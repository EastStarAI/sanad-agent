...
- Web callback delivery validates exact Portal origin, popup source, message type, and transaction state. Desktop binds literal IPv4 loopback on an ephemeral port and fixed path, then returns a no-store completion page that mirrors the Portal Success design, attempts to close, and otherwise tells the user to return to Sanad. Any external visual asset requires a no-referrer policy so the callback URL cannot leak. Mobile normally accepts only environment-configured claimed HTTPS links; iOS Debug builds may use the exact Development-only `sanad://oauth/ios-development` callback when the explicit Development config is compiled in.
- Android owns verified App Links for `/oauth/android` on the Development, Staging, and Production Portal hosts. iOS Profile/Release builds own `/oauth/ios` on the matching Client hosts so the final provider callback navigation crosses from Portal to Client instead of being retained by Safari's same-domain Universal Link rule. The Development custom-scheme path is selected only by iOS Debug with exact `ENVIRONMENT=dev`; it retains S256 PKCE, state validation, and one-time code redemption. `app_links` is the sole Flutter callback handler and subscribes before browser launch so cold/warm delivery is not lost; iOS must disable Flutter's built-in Router deep-link handler to prevent callback ownership races. Association documents authorize only `com.eaststarai.sanad`; Android certificate fingerprints are environment-injected public metadata and missing configuration fails closed.
...
- Refresh and logout must use the portal-owned operations; never rotate tokens through the backend gateway. Account lifecycle inventory and targeted revoke are authenticated Cloud Socket operations and must not be added to `PortalAuthClient`.
- Logout is single-flight across direct UI logout, current-session revoke result, and terminal socket revocation so concurrent trusted signals perform one credential cleanup and Portal revoke request.
...
- Native desktop auth mutations publish only a credential-free exchange request after `auth.json` persistence. Incoming exchange reconciliation reloads that file and never echoes another request. Keep profile retrieval and other authenticated application reads outside `auth.refresh.lock`; only the auth read-modify-write transaction belongs inside it. Web and mobile do not participate.
