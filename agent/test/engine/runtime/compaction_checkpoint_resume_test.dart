...
void main() {
  ...
  setUp(() async {
    ...
    runner = AgentRunner(
      ...
      ToolsRegistry(),
      sessionManager,
      existingSessionId: 'session-checkpoint',
    );
    runner.debugSaveCheckpointForTesting(
      resumeHistoryLength: runner.history.length,
      turnStartIndex: 1,
    );
  });
  ...
  test('checkpoint resume stays valid after compaction activation', () async {
    ...
    expect(runner.history.length, canonicalLengthBefore);
  });
}
