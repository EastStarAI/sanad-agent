...
- After an authenticated Local WebSocket upgrade, a new Client may send one bounded `client.hello` with protocol/version, Client instance UUID, allowlisted display metadata, and capabilities. The hello is correlation-only, grants no authorization, and an absent/invalid hello cannot claim local delivery suppression; preserve legacy socket operation for older Clients.
