...
- `core/setup/` owns typed service state, platform capability selection, service-definition generation, lifecycle execution, ownership metadata, conflict migration, and transactional restore; entry points only validate commands and present results.
...
- Persist Agent private keys and durable credentials through the platform store abstraction: macOS Keychain, Windows DPAPI-protected ciphertext, or Linux capability-selected Secret Service/owner-protected storage. Linux probes Secret Service with a synthetic write/read/delete, persists one backend identity, and otherwise uses the atomic owner-only Sanad Home file backend without claiming encryption. A selected backend never reads stale credentials from another backend; migration to Secret Service deletes owner-file bytes only after every write/read verification succeeds. Legacy files are removed only after verified migration. Device code, pairing authority, pending credentials, and proof JWTs never enter `auth.json`, URLs, Agent CLI arguments, or logs. Replacement pairing snapshots a prior Device Credential only in the selected secret backend; cancellation restores it atomically, while authoritative registration discards the snapshot.
