...
- Refresh `sanad_client` User access through the Portal lifecycle, send refresh tokens in request bodies, and never log raw credentials. Headless Agent login instead owns a P-256 identity and stores only a key-bound `sanad_agent` Device Credential; it never receives a User refresh token.
