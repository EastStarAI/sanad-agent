...
## Aggregate Owners
- `SessionWorkItemRepository` owns durable work-item CRUD, FIFO claim, transitions, route rewrites, orphan-row SQL, terminal-row prune SQL, and cancellation. It exposes identity discovery plus bounded conditional delete batches; post-ready timing, idle gating, and retention policy belong to `AgentStateMaintenanceService`, not this aggregate.
...
- Pending-steer revisions validate run and generation; history persistence, delivered message/turn identity, causal anchor, history revision, and the delivered transition commit atomically. Only delivery failure may return delivering to pending.
