...
Rules:
- OAuth providers (`openai-codex`, `github-copilot`, `xai-oauth`, `nous`) must not store long-lived tokens in `.env`.
...
`github-copilot` is an instance-scoped subscription template (`authType: oauth_external`, `authFlow: device_code`, `apiMode: chat_completions`, `auth_methods: [device_code]`). Aliases are `copilot` and `github-copilot`. There is no API-key path and no external Copilot CLI, GitHub CLI, or SDK: Sanad owns the agent loop and talks to GitHub over Dart `http.Client`.

Central non-secret constants live in `GithubCopilotProtocol`: public Client ID `Iv1.b507a08c87ecfe98`, OAuth scope `read:user`, GitHub device-code URLs, `https://api.github.com/copilot_internal/v2/token`, default API host `https://api.githubcopilot.com`, and the static request headers `Copilot-Integration-Id: vscode-chat`, `Openai-Intent: conversation-edits`, and `Editor-Version: vscode/1.104.1`. Classic personal access tokens (`ghp_*`) are rejected with a typed error; they are not a supported Copilot credential.

The typed exchange DTO is `CopilotTokenExchangeResult` (`token`, `expiresAt` epoch milliseconds, optional `accountEndpoint`). After a successful GitHub device-code approval, the GitHub user token is stored as the instance `refreshToken` (long-lived exchange source) and the Copilot API token is stored as `accessToken` with `expiresAt`. The DTO prefers `endpoints.api` and otherwise derives a host from the token `proxy-ep` hint (`proxy.` → `api.`). Only HTTPS hosts on `*.githubcopilot.com`, `copilot-proxy.githubusercontent.com`, or an explicit trusted GHE tenant (`tenant.ghe.com`) are stored; any other host is dropped so the instance keeps the default API base. Rejected advertised endpoints must not fall through to `proxy-ep`. Tokens never appear in logs, summaries, or protocol payloads.

The GitHub device-code flow is owned by `ProviderAuthSessionService`. `poll()` is provider-aware: Codex and Copilot sessions are dispatched independently. RFC 8628 `authorization_pending` stays pending, `slow_down` raises the poll interval by 5 seconds (never lowering it), `expired_token` expires the session, and `access_denied` ends with a typed error. `cancel` marks the in-flight session so the next poll returns `cancelled` without writing secrets. Classic `ghp_*` tokens are rejected before exchange. The `SecretRecord` is written only after both GitHub approval and Copilot-internal exchange succeed. A validated `accountEndpoint` is applied to that instance's `baseUrl` only. The template is visible in `ProviderCatalogService`.

Copilot access-token refresh is instance-scoped and owned by `CopilotCredentialLifecycle`. Before a model call, `CopilotAuthRecoveryAdapter` refreshes when `expiresAt` is within 120 seconds (`now >= expiresAt - 120_000`). Concurrent callers for the same instance share one in-flight exchange. A Copilot HTTP 401 triggers exactly one forced exchange and one retry; a second 401, a missing GitHub user token, a classic PAT, or a permanent exchange failure (HTTP 401/403/404) persists `relogin_required` and does not retry again. Transient exchange failures do not flip that status. Refresh writes a new `accessToken`/`expiresAt` and bumps `credentialRevision` on that instance only.

Model requests add the static Copilot headers (`Copilot-Integration-Id`, `Openai-Intent`, `Editor-Version`) plus per-call overlays: `x-initiator` is `user` on the first model request of a turn and `agent` after tool results; `Copilot-Vision-Request: true` is sent only when the request history contains image or vision input. Chat Completions models use `BaseOpenAIAdapter`. Models whose id names the Responses protocol (a `responses` path segment) use `CodexResponsesAdapter` against the instance `baseUrl`, including a discovered account endpoint. Streaming and tool calling stay on those adapters.

Live models come from `GET ${baseUrl}/models` with the Copilot bearer token and static headers. Sanad keeps models that can stream and call tools, and drops SKUs whose `policy.state` is not `enabled` or that are hidden from the picker. When the live list is empty or unreachable, the template fallback list (`claude-sonnet-4.6`, `claude-haiku-4.6`, `gpt-5.4`, `gpt-4o`) is used. RFC 8628 `slow_down` raises the device-code poll interval on both the CLI and the Flutter setup UI.
