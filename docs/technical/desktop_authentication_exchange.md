...
A refresher acquires the lock before re-reading `auth.json`. If the persisted access/refresh pair changed while it waited, it adopts that complete pair and does not call the Portal. Otherwise it keeps the lock through the bounded Portal request and atomic pair persistence. Login, logout, pairing, and auth-document read-modify-write mutations use the same lock to prevent lost updates. Profile retrieval and other authenticated application reads are not part of the mutation transaction and start only after the Client releases the lock. Web and mobile neither open `auth.json` nor participate in this lock.

Cloud Gateway reconnect registration reloads Agent authentication through this same lock. An acquisition timeout is transient: the cloud adapter contains the failure, waits briefly while the socket remains connected, and restarts registration without a challenge nonce so the Gateway issues a fresh challenge. Concurrent reconnect and challenge callbacks are coalesced; lock contention cannot terminate the daemon event loop.
