...
- **سجل التقدم:**
  ...
  - فحص `gitleaks` على commit الدمج صنّف قيمتي canary اصطناعيتين في اختبارات
    redaction كـ`generic-api-key`. أضيف المساران المحددان فقط إلى allowlist
    الاختبارات الاصطناعية الموجودة، دون توسيع القواعد أو استثناء كود المنتج.
