...
canonical installer sources. The current patch release uses marketing version `1.0.10` and build number `11` for both Sanad Agent and Sanad Client. RC tags use `v1.0.10-rc.N`; Stable uses `v1.0.10`. The unpublished `v1.0.3` candidate tag is retained as failed provenance after its Windows smoke gate rejected a PowerShell reserved-variable collision; it owns no GitHub Release or Production asset. Each later candidate records its increasing build number in the checked-in contract.
