...
canonical installer sources. The current patch release uses marketing version `1.0.3` and build number `4` for both Sanad Agent and Sanad Client. RC tags use `v1.0.3-rc.N`; Stable uses `v1.0.3`. Each later candidate records its increasing build number in the checked-in contract.
