...
|---|---|---|---|---|---|
| Tool edit/read p50/p95 and event-loop lag P06 | Baseline: synchronous tool execution blocks microtask/event queue | Event loop unblocked; independent query completes before slow tool terminal result; lag budget p50 < 50 ms, p95 < 200 ms | 97f: event loop unblocked; independent socket query arrives 22.85 ms before slow tool completes; lag p50: 10.35 ms, p95: 11.40 ms | 30 warm samples (97f) | Pass (meets <50ms/<200ms lag budget with >75% headroom) |
| History load p50/p95, idle vs tool-active | Baseline: atomic append + cached revision in c19bd57 | Zero history read lock contention during tool execution; idle load p50 < 10 ms, p95 < 25 ms; tool-active load p50 < 20 ms, p95 < 50 ms | 97f: Zero DB lock contention; Idle: p50 1.22 ms, p95 1.90 ms; Tool-active: p50 0.73 ms, p95 1.25 ms | 30 idle + 30 tool-active samples (97f) | Pass (meets <10ms/25ms idle and <20ms/50ms active budgets) |
| Provider readiness p50/p95 P08 | Baseline: readiness previously conflated with absence; serial provider calls ~6.54s (instances ~2.48s + runtime_check ~1.50s + usage ~2.12s) caused by PowerShell SDDL subprocess per read | Distinct loading visible within 50 ms; 0 false absent flashes; startup resolution p50 < 500 ms, p95 < 1,500 ms; parallel overlap eliminates serial wait; Win32 FFI eliminates subprocess latency | 97g: Root-cause PowerShell ACL subprocess eliminated via in-process Win32 FFI (`advapi32.dll`/`kernel32.dll` in `WindowsSecureRuntimeBackend`); 50 secret reads/writes reduced from 35s to <2ms. Live Windows Agent `provider.runtime_check` dropped from ~1,720ms to 4ms–14ms (>100x speedup). Client parallel `FutureRecord3.wait` overlaps `templates` + `instances` + `runtimeCheck`. Client neutral checking state and HomeScreen gating prevent any false "Add provider" or "Provider setup required" flash. | Live driver samples on Windows (.sanad-test) with `sanad-dev restart client` + phase microbenchmark (30 samples) + 21 agent bootstrap tests + 63 client unit/widget tests + 2 widget readiness tests | Pass (Win32 FFI in-process ACL, serial overhead eliminated; runtime_check 4-14ms; 0 false absent flashes; full multi-platform interactive matrix deferred to 97l) |
| Delayed/empty/error/reconnect inventory P09 | Baseline: phone empty state (`No devices connected`) rendered while inventory fetch still in flight (false absent flash); fetch errors rethrew and could drop the surface | Loading/stale/error/authoritative-empty distinct; timeout is error, never authoritative empty; no-devices flash eliminated; fetch error retains valid data and never logs out | 97g: typed `DevicesPhase` (loading/ready-empty/ready/error/stale); static `Checking for your devices…` loading state during fetch; `Couldn't load your devices` + Retry on failure; error preserves cached agents and active device; no logout | `device_cubit_test.dart` (typed phase + timeout + error-retention + live-session) and `onboarding_setup_choices_test.dart` (phone loading→empty/loading/error) on Windows | Pass (13 device-cubit + 6 widget tests); real reconnect/delayed-old-device trace deferred to 97l |
...
| Resize/rebuild extra fetches P02 | User log: 2 duplicate pairs (`provider.usage.support`, `model.snapshot` in 162ms) | Exactly 0 extra Agent fetches for unchanged logical resources across 20 resize/rebuild cycles | 97h: catalog + support + usage owned by `ProviderUsageCubit`; 20 remount/theme/resize/rebuild cycles keep request counts at initial (widget test + cubit re-entry tests) | 20 resize cycles + rebuild/remount (97h) | Pass (deterministic count tests; interactive resize in 97l) |
| Equivalent simultaneous request P03 | Baseline: concurrent duplicate fetches observed on remount | Exactly 1 in-flight request per logical resource key (device/query scope); 100% request deduplication | 97h: two consumers of same in-flight (device, instance) share one `usage.get`/`support`; different devices/queries never share or cancel; catalog shared per device | deterministic unit/widget test (97h) | Pass (share + per-device isolation tests) |
| Request counts/bytes per action P04/P05 | User log: `get_sessions` repeated 9 times in 15ms | Exactly 1 fetch per workspace section/cursor; 0 redundant duplicate calls within 500ms debounce | 97i: Coalesced in-flight requests for workspaces, sidebar, sections, and cursors in `ConversationCacheRepository`; 500ms debounce window prevents rapid duplicate triggers; store `applySectionPageAppended` clears `nextCursor` when pagination completes | deterministic unit test (8 cases in `conversation_cache_repository_pagination_efficiency_test.dart`) | Pass |
| Page sizes current 6/10 vs experiment 9/15 | Current default: initial 6, subsequent 10 | Adopt +50% page size (9/15) only upon payload overhead <15% and scroll request drop >=30% | 97i: Experiment conducted across single section, 8-workspace sidebar, and weighted 100-section scroll distribution. Initial payload overhead measured +48.9% (single: 2,314B vs 3,445B) and +25.4% (sidebar: 16,366B vs 20,521B), exceeding <15% ceiling; request reduction was only 13.3% (158 vs 137 requests), failing >=30% threshold. Increase rejected with documented decision; default 6/10 retained | 4 test cases in `conversation_pagination_page_size_experiment_test.dart` | Documented Rejection / Pass |
| Secure write p50/p95 P10 | Historical (PowerShell): >12,000 ms. Win32 FFI baseline (a087238): cold 34.38 ms, warm p50 4.63 ms, p95 8.54 ms | <2,000 ms median or >=50% improvement | 97a Win32 FFI atomic write: p50 4.63 ms, p95 8.54 ms. Independent secure-runtime characterization (PR #172 / 97c): p50 <17 ms, p95 <22 ms across new-directory, atomic-file, existing-file, append and read operations (detail below) | 30 warm samples (min 3.82ms, max 9.55ms) + 1 cold (97a); 1 cold + 30 warm samples for the PR #172 characterization | Pass (>99% reduction from historical PowerShell baseline; meets <2,000 ms budget with >1,980 ms headroom) |
...
97f deterministic responsiveness suite (implemented): `agent/test/capabilities/file_tool_responsiveness_test.dart`, run with `fvm dart test test/capabilities/file_tool_responsiveness_test.dart`. It covers: independent socket query reception and completion before a slow/heavy file tool finishes; event-loop microtask lag during file edit/read (30 samples); SQLite history query latency under concurrent heavy tool execution (30 samples, 0 lock contention); full_access and defaultMode external path permission verification without workspace-only regression; atomic write ordering; CRLF newline integrity; and asynchronous directory listing pagination.

97i deterministic pagination efficiency and experiment coverage is owned by `client/test/unit/repositories/conversation_cache_repository_pagination_efficiency_test.dart` and `client/test/unit/repositories/conversation_pagination_page_size_experiment_test.dart`. It covers: coalescing of concurrent in-flight unscoped and workspace-scoped refreshes to exactly 1 transport fetch; independent isolation across different workspaces; coalescing of concurrent in-flight `loadMore` calls for the same section/cursor with proper await until append completion; coalescing of `refreshWorkspaces` and `refreshDeviceSidebar`; 500ms debounce gating for non-forced rapid duplicate requests; store `nextCursor` clearing on page append completion; and the empirical 6/10 vs 9/15 benchmark measuring request counts, payload bytes, memory object allocations, and scroll frequency drops against the frozen 97a adoption policy.
