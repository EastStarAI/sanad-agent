...
| Provider readiness p50/p95 P08 | Baseline: readiness previously conflated with absence; serial provider calls ~6.54s (instances ~2.48s + runtime_check ~1.50s + usage ~2.12s) | Distinct loading visible within 50 ms; 0 false absent flashes; startup resolution p50 < 500 ms, p95 < 1,500 ms; parallel overlap eliminates serial wait | 97g: Agent phases microseconds (catalog p50 89µs, setupStatus p50 390µs, runtimeCheck p50 810µs). Client parallel `FutureRecord3.wait` overlaps `templates` + `instances` + `runtimeCheck`: 3-query snapshot p50 3.03s, p95 4.52s; end-to-end provider flow p50 5.01s (23.4% reduction from 6.54s baseline), p95 6.41s; 1 request per query, 0 duplicate/unhandled errors | 3 live driver samples on Windows (.sanad-test) + phase microbenchmark (30 samples) + client unit/widget (63 tests) | Pass (serial overhead eliminated; p50 5.01s vs 6.54s baseline; client loading distinct; full multi-platform interactive matrix deferred to 97l) |
