...
| `run --background` from a temporary non-TTY shell or Windows tool shell subject to a kill-on-close Job Object | On POSIX, a detached launcher retains the complete managed process group after requester shell exit. On Windows, CIM `Win32_Process.Create` breaks out of the enclosing kill-on-close Job Object; launcher, lease, and components remain alive after shell termination. |
| Background requester observes a new managed/failed attempt | It returns zero only after proving the launcher process is alive and requested components are actively managed; diagnostic attempt `managed` alone never causes premature completion. Staged failure or bounded timeout returns nonzero with a status recovery hint. |
| Detached child publishes failure or crashes while exiting | A two-second post-exit publication grace lets the atomic attempt/locator win over PID polling, and the requester prints the exact stage, bounded reason, and exit status without declaring premature success. |
...
| Interactive `run all` | Current terminal mirrors Agent and accepts `r`/`R` for supervised restart plus `s`/`q` for resumable Agent-only stop; one independent terminal opens for each Client and routes Flutter `r`/`R`/`h`/`d`/`c`/`q` to the exact owned process. |
| Interactive `run client`, including adding Client to an Agent-only managed runtime | Client output remains in the invoking terminal; no second terminal opens, and the complete Flutter interactive command set is routed to the exact launcher-owned process. |
| Automatically opened Client journal terminal | The surface prints at most the latest 50 retained lines, follows new output through the complete five-minute cold-build decision, and returns to the shell only after the Client exits, detaches, fails, or is stopped; explicit `logs` history selection remains caller-controlled. |
...
| Client cold desktop build exceeds 90 seconds but completes within five minutes | Launch, managed component start, source switch, rollback, and manual restoration continue waiting; the launcher does not terminate the Client or report `BUILD INTERRUPTED`. |
