...
| Linux verified backend migration | A later usable Secret Service receives and reads back every owner-file entry before backend metadata changes and owner-file bytes are deleted. Any unverifiable entry preserves the owner-file backend and bytes. |
| Linux auth.json minimization | Device Credential, pending Device Credential, P-256 private key, and pairing authority are absent during pending pairing and after restart; legacy secret fields are removed only after selected-backend read-back. |
...
| Windows legacy migration and corruption | Legacy `auth.json.device_token` is removed only after DPAPI read-back verification. Corrupt DPAPI ciphertext leaves local startup healthy, clears Agent cloud authority, and never becomes plaintext. |
| Windows clock skew | With the workstation clock materially different from Portal time, co-located login calibrates proof `iat` from the authenticated HTTPS `Date` response, persists only the non-secret offset in DPAPI, and both Device Authorization and Gateway challenge proofs pass after daemon restart. After the operating-system clock is corrected, the next Gateway registration refreshes the offset to current Portal time before signing. Non-HTTPS or malformed dates are ignored. |
| Windows installer rollback | The warning derives its version from the verified manifest. A failed first installation removes a Scheduled Task created by that attempt, restores the previous executable when present, and retains Sanad Home. |
...
Run the shared lock package tests, including the real two-process contention fixture. Real Linux Secret Service session tests require `SANAD_TEST_LINUX_SECRET_SERVICE_REAL=1` and an unlocked user-session service; ordinary CI keeps deterministic unavailable-service coverage without assuming desktop D-Bus infrastructure. Run focused agent tests for `AuthManager`, Local Gateway transport, and cloud platform authentication lifecycle. Run focused Flutter tests for `AuthService` and socket recovery, plus the daemon-backed `Client logout reaches Local Gateway and deletes Agent vault authorization` case sequentially, then both analyzers. The integration case must use a temporary Sanad Home, migrate a synthetic Device Credential into the platform Secret Store, logout through the real Client adapter, restart the daemon, and prove authorization remains absent. Windows, Linux, and macOS release CI must execute the shared package test before release closure.
