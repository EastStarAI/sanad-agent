...
| OpenCode conversation affinity | Every conversation-bound OpenCode request sends `x-opencode-session` from the immutable durable session id in sync and stream paths. Official templates and exact `opencode.ai` custom endpoints are covered; unrelated and deceptive-suffix hosts receive no header. |
...
|---|---|
| Agent unit tests | Readiness promotion waits for verified cache state; Base URL normalization covers legacy copied labels, malformed/unsupported schemes, and canonical `/v1/models`; model-cache coverage proves fallback diagnostics, truthful no-cache failure, coalesced-owner cleanup, and a fresh retry after failure. OAuth identity extraction covers claim priority, invalid values, lazy stored-record enrichment, summary mapping, and instance-keyed approval persistence. OpenCode affinity coverage proves stable and isolated session headers across OpenAI, Anthropic, and Responses sync/stream paths, exact-host custom endpoints, and non-OpenCode exclusion. |
