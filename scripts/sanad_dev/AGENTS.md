...
* **Terminal Ownership:** `run client` keeps Client output and Flutter-style `r`/`R` controls in the invoking terminal. `run agent` and the current terminal of `run all` expose Agent output with `r`/`R` restart control; only `run all` opens a separate Client terminal. Client sidecars may route `r`/`R` through the owning launcher but never own the Flutter process. Terminal adapter failure or headless execution must not change process ownership or runtime success.
