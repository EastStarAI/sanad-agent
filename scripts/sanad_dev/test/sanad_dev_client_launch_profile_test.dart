...
import 'package:sanad_dev/client_launch_profile.dart';
...
void main() {
  ...
  test('derives a bounded preference namespace for an additional Client', () {
    expect(
      ...
      'sanad.abcd.instance.49h-local2.',
    );
    expect(
  ...
  test('validates an additional Client slot against its derived namespace', () {
    final profile = extractClientLaunchProfile([
      ...worktreeArguments,
      '--dart-define=$sanadDevClientInstanceSlotDefine=49h-local2',
      '--dart-define=SANAD_SHARED_PREFERENCES_PREFIX=sanad.abcd.instance.49h-local2.',
    ]);
  ...
  test('extracts exact argv values including paths with spaces', () {
    ...
    expect(profile.define('SANAD_SHARED_PREFERENCES_PREFIX'), 'sanad.abcd.');
    expect(
  ...
  test(
    'implicit primary defaults never override explicit conflicting identity',
    () {
      final discovered = extractClientLaunchProfile([
        'flutter',
        'run',
  ...
  test(
    'implicit primary defaults remain disabled for unsafe runtime identity',
    () {
      final discovered = extractClientLaunchProfile([
        'flutter',
        'run',
  ...
  test(
    'resolves linked and primary client directories without using target path',
    () {
      ...
      final linkedDirectory = resolveClientDirectoryForLaunchProfile(
        ...
        runtimeRepositoryRoot:
            '/repo/.agent/worktrees/58-safe-restart-recovery/sanad-agent',
      ...
      final spacedDirectory = resolveClientDirectoryForLaunchProfile(
        profile: linkedProfile,
        runtimeRepositoryRoot:
            '/repo with spaces/.agent/worktrees/58-safe-restart-recovery/sanad-agent',
        runtimeIsLinkedWorktree: true,
        runtimeWorktreeName: '58-safe-restart-recovery',
        separator: '/',
      );
  ...
  test(
    'attach arguments preserve launch profile and never inject local config',
    () {
      final profile = extractClientLaunchProfile(worktreeArguments);
      ...
      expect(
        arguments,
        isNot(contains('--dart-define-from-file=config/local.json')),
      );
    },
  );
  ...
  test('standalone isolated profile requires the matching workspace hash', () {
    ...
    String? validate(ClientLaunchProfile profile) =>
        validateClientLaunchProfile(
          profile,
          isLinkedWorktree: false,
  ...
  test('null-delimited argv preserves paths with spaces', () {
    final arguments = splitNullTerminatedArguments(
      'flutter\u0000run\u0000--dart-define=SANAD_HOME=/Users/Dev User/.sanad\u0000'
          .codeUnits,
    );
  ...
  test('linked global or custom home remains valid with derived prefix', () {
    ...
    final error = validateClientLaunchProfile(
      ...
      expectedAgentPort: 58092,
      emptyPreferencesSanadHome: '/users/dev/.sanad',
      derivePreferencesPrefix: (home) =>
          home == '/tmp/worktree home' ? 'sanad.abcd.' : 'wrong.',
      canonicalizePath: (path) => path,
    );
    expect(error, isNull);
  });
  ...
  test('primary explicit custom home requires its derived namespace', () {
    ...
    final error = validateClientLaunchProfile(
      ...
      emptyPreferencesSanadHome: '/users/dev/.sanad',
      derivePreferencesPrefix: (home) =>
          home == '/custom/sanad home' ? 'sanad.custom.' : 'wrong.',
      canonicalizePath: (path) => path,
    );
    expect(error, isNull);
  });
  ...
  test(
    'macOS candidate seam invokes native argv reader for every pid',
    () async {
      final invoked = <int>[];
  ...
  test(
    ...
    () {
      ...
      expect(
        arguments.join(' '),
        contains('.dart_tool/pub/bin/test/test.dart'),
      );
    },
