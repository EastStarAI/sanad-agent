...
import 'package:sanad_dev/runtime_switch.dart';
...
void main() {
  ...
  test('active switch ownership distinguishes live and stale launchers', () {
    ...
    expect(isActiveRuntimeSwitch(request.copyWith(status: 'failed')), isFalse);
  });
  ...
  test('already-target classification requires Agent and Client agreement', () {
    final targetClient = ['target', 'client'].join(Platform.pathSeparator);
    final previousClient = ['previous', 'client'].join(Platform.pathSeparator);
  ...
  test(
    'fromJson does not validate target root if status is not requested',
    () async {
      final home = await Directory.systemTemp.createTemp('sanad-switch-home');
      addTearDown(() => home.delete(recursive: true));
      final path = runtimeSwitchManifestPath(home.path, 58091);
  ...
  test('switched client group preserves every device and VM port', () {
    ...
    expect(
      groups[0],
      containsAll(['-d', 'macos', '--host-vmservice-port=51123']),
    );
    expect(
      groups[1],
      containsAll(['-d', 'iphone-simulator-id', '--host-vmservice-port=51124']),
    );
