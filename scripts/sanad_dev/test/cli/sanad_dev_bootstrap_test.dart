...
void main() {
  ...
  setUp(() async {
    ...
    ).writeAsString(await File('../sanad-dev').readAsString());
...
''');
  ...
  group('POSIX bootstrap', () {
    test('fresh setup resolves Contract before Agent and Client', () async {
      ...
      expect(invocations[2], contains('scripts/sanad_dev|dart pub get'));
      expect(invocations[3], contains('agent|dart pub get'));
    ...
    test(
      'second setup skips dependency resolution with a valid stamp',
      () async {
        ...
        expect(
          result.stdout,
          isNot(contains('Resolving package dependencies')),
        );
        final invocations = await calls.readAsLines();
    ...
    test(
      ...
      () async {
        ...
        expect(
          await Link(
            '${userBin.path}${Platform.pathSeparator}sanad-dev',
          ).exists(),
          isTrue,
        );
      },
    ...
    test(
      'run accepts an existing command shim owned by another checkout',
      () async {
        final shim = Link('${userBin.path}${Platform.pathSeparator}sanad-dev');
        final foreign =
    ...
    test(
      'switch prepares an unready target before entering runtime CLI',
      () async {
        final shim = Link('${userBin.path}${Platform.pathSeparator}sanad-dev');
        final foreign =
            '${fixture.path}${Platform.pathSeparator}other-checkout';
    ...
    test('failed switch preparation never enters runtime CLI', () async {
      ...
      expect(
        invocations.any((line) => line.contains('sanad_dev_cli.dart switch')),
        isFalse,
      );
    });
    ...
    test('foreign wrapper redispatches to the caller Git worktree', () async {
      ...
      await foreignWrapper.writeAsString(
        await File('../sanad-dev').readAsString(),
      );
      await File(
    ...
    test('checkout collision fails unless force is explicit', () async {
      ...
      expect(
        await shim.target(),
        contains(
          '${Platform.pathSeparator}scripts${Platform.pathSeparator}sanad-dev',
        ),
      );
    });
    ...
    test('failed stage blocks dependent setup and runtime launch', () async {
      ...
      expect(invocations.last, contains('release/contract|dart pub get'));
      expect(
        invocations.any((line) => line.contains('flutter pub get')),
        isFalse,
      );
      expect(
        invocations.any((line) => line.contains('sanad_dev_cli.dart')),
  ...
  test('Windows setup executes all stages with a user-scoped shim', () async {
    ...
    expect(
      invocations.any(
        (line) => line.contains(
          'release${Platform.pathSeparator}contract|dart pub get',
        ),
      ),
      isTrue,
    );
    expect(
      invocations.any(
        (line) => line.contains(
          'scripts${Platform.pathSeparator}sanad_dev|dart pub get',
        ),
      ),
      isTrue,
    );
    expect(
      invocations.any((line) => line.contains('agent|dart pub get')),
      isTrue,
    );
    expect(
