...
const String canonicalPrimaryGatewayUrl =
    'http://127.0.0.1:$canonicalPrimaryAgentPort';
const String sanadDevClientInstanceSlotDefine =
    'SANAD_DEV_CLIENT_INSTANCE_SLOT';

String sanadDevPreferencesPrefixForClientInstance(
  String basePrefix,
  String? instanceSlot,
) {
  final slot = instanceSlot?.trim() ?? '';
  if (slot.isEmpty) return basePrefix;
  if (!RegExp(r'^[a-z0-9][a-z0-9-]{0,31}$').hasMatch(slot)) {
    throw const FormatException(
      'Client instance must use 1-32 lowercase letters, digits, or hyphens.',
    );
  }
  return '${basePrefix}instance.$slot.';
}
...
String? validateClientLaunchProfile(
  ...
  final basePreferencesPrefix =
      canonicalize(sanadHome) == canonicalize(emptyPreferencesSanadHome)
      ? ''
      : derivePreferencesPrefix(sanadHome);
  String expectedPreferencesPrefix;
  try {
    expectedPreferencesPrefix = sanadDevPreferencesPrefixForClientInstance(
      basePreferencesPrefix,
      profile.define(sanadDevClientInstanceSlotDefine),
    );
  } on FormatException catch (error) {
    return error.message;
  }
  if (preferencesPrefix != expectedPreferencesPrefix) {
    return 'SANAD_SHARED_PREFERENCES_PREFIX does not match SANAD_HOME and client instance';
  }
  final workspaceHash = profile.define('SANAD_DEV_WORKSPACE_HASH');
  if (workspaceHashRequired &&
      (workspaceHash == null || workspaceHash.isEmpty)) {
    return 'SANAD_DEV_WORKSPACE_HASH is missing';
  }
  if (expectedWorkspaceHash != null &&
      workspaceHash != null &&
...
String resolveClientDirectoryForLaunchProfile({
  ...
  final normalizedRoot = runtimeRepositoryRoot
  ...
  final primaryRepositoryRoot = markerIndex >= 0
      ? normalizedRoot.substring(0, markerIndex)
      : normalizedRoot;
...
List<String> readMacOsProcessArguments(
  ...
  try {
    ...
    mib[2] = processId;
    sizePointer.value = 0;
    if (sysctl(mib, 3, nullptr, sizePointer, nullptr, 0) != 0 ||
        sizePointer.value <= 0) {
      return const [];
    }
